...
export const Header: React.FC<HeaderProps> = ({
  ...
  const isRecordings = currentPath.startsWith('/recordings');
  const canOpenSelectedClassroom = canAccessClassroom(session, selectedCourse.id);
  const canOpenSelectedRecordings = canAccessRecordings(session, selectedCourse.id);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 gap-4">
          ...
          <nav className="hidden md:flex items-center gap-1.5">
            ...
            {/* Recordings -> /recordings/:slug (trước Lớp học) */}
            <button
              onClick={() => {
                if (canOpenSelectedRecordings) {
                  onNavigate(recordingsPathFor(selectedCourse));
                } else {
              ...
              title={
                canOpenSelectedRecordings
                  ? `Xem recordings ${selectedCourse.title}`
                  : 'Chưa có quyền recordings — mở Overview'
              }
            >
        ...
        <div className="md:hidden bg-white border-b border-slate-200 px-4 pt-2 pb-4 space-y-2 shadow-lg">
          ...
          <div className="space-y-1">
            ...
            <button
              ...
              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-bold ${
                isRecordings ? 'bg-[#FFE3E9] text-[#e34e6b]' : 'text-slate-700'
              }`}
            >
              <Video className="w-4 h-4" />
